...
import { CheckCircle2, TrendingUp, Building2, Sparkles, ScrollText, Users, Clock, ShieldAlert } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
      ...
      {/* Navigation Tabs */}
      <div className="flex items-center gap-4 border-b border-slate-200 mb-6 overflow-x-auto no-scrollbar">
        {[
      ...
      {loading ? (
        <div className="py-16 text-center">
          <div className="w-8 h-8 border-2 border-brand-forest border-t-transparent rounded-full animate-spin mx-auto mb-2" />
          <p className="text-xs text-slate-500 uppercase tracking-wider">Syncing Admin Console...</p>
        </div>
      ) : activeTab === 'orders' ? (
        /* Orders Tab */
        <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-card-soft">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="p-4 font-bold text-slate-900 font-serif">
                      {order.orderRef}
                    </td>
                    <td className="p-4">
                      <div className="font-medium text-slate-900">{order.contactName}</div>
                      <div className="text-[11px] text-slate-400">{order.contactEmail}</div>
                    </td>
                    ...
                    <td className="p-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                        order.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' : 'bg-brand-forest/10 text-brand-forest border border-brand-forest/20'
                      }`}>
                        {order.status}
                      </span>
                    </td>
                    <td className="p-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                        order.paymentStatus === 'COMPLETED' ? 'bg-emerald-700 text-white' : 'bg-brand-terracotta/15 text-brand-terracotta'
                      }`}>
                        {order.paymentStatus}
                      </span>
                    </td>
                    <td className="p-4 font-bold text-brand-forest whitespace-nowrap font-serif">
                    ...
                    <td className="p-4 text-right">
                      <select
                        ...
                        className="px-2.5 py-1.5 bg-white border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:border-brand-forest"
                      >
            ...
            >
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <span className="text-[10px] uppercase font-bold text-brand-terracotta">{p.cuisine}</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                    p.isApproved ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' : 'bg-brand-terracotta/15 text-brand-terracotta'
                  }`}>
                ...
                <p className="text-xs text-slate-500 line-clamp-2 mb-3 font-sans">
                  {p.description}
                </p>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="p-4 font-bold text-[11px] text-brand-terracotta">
                      {u.role}
                    </td>
                    <td className="p-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {aiLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="p-4 font-bold text-brand-forest font-serif">
                      ₹{log.budgetPerHead}
                    </td>
                    ...
                    <td className="p-4">
                      <span className="font-semibold text-brand-terracotta">{log.provider}</span> ({log.model?.split('/')[1] || log.model || 'fallback'})
                    </td>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-card-soft">
          <div className="p-5 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h3 className="font-serif text-sm font-bold text-slate-900">Partner Guild Onboarding Verification</h3>
              <p className="text-[11px] text-slate-500">Review official compliance credentials, FSSAI licenses, and settlement information submitted by restaurant partners.</p>
            </div>
            <span className="text-xs font-bold text-brand-terracotta whitespace-nowrap">{applications.length} Total Submissions</span>
          </div>
                ...
                {applications.length === 0 ? (
                  ...
                  applications.map((app) => (
                    <tr key={app.id} className="hover:bg-slate-50/70 transition-colors">
                      <td className="p-4">
                        <div className="font-bold text-slate-900 text-sm font-serif">{app.data?.restaurantName || 'Unnamed Kitchen'}</div>
                        <div className="text-[11px] text-brand-terracotta font-medium">{app.data?.restaurantType || 'Heritage Restaurant'}</div>
                        <div className="text-[10px] text-slate-400 font-mono mt-0.5">ID: {app.id?.slice(0, 14)}...</div>
                      </td>
                      <td className="p-4">
                        <div className="font-medium text-slate-900">{app.data?.city || 'Hyderabad'}</div>
                        <div className="text-[11px] text-slate-500 max-w-xs truncate">{app.data?.address || 'Address provided'}</div>
                        <div className="text-[10px] text-slate-400 mt-0.5">{app.data?.outletsCount || '1'} Outlet(s)</div>
                      </td>
                      <td className="p-4">
                        <div className="font-medium text-slate-900">{app.data?.ownerName || 'Authorized Rep'}</div>
                        <div className="text-[11px] text-slate-500">{app.data?.ownerMobile || app.data?.contactNumber || '—'}</div>
                        <div className="text-[10px] text-slate-400">{app.data?.ownerEmail || app.data?.email || '—'}</div>
                      </td>
                      <td className="p-4">
                        <div className="font-mono text-[11px] text-brand-forest">FSSAI: {app.data?.fssaiNumber || 'Pending'}</div>
                        <div className="font-mono text-[10px] text-slate-400">PAN: {app.data?.panNumber || '—'}</div>
                        <div className="font-mono text-[10px] text-slate-400">GST: {app.data?.gstin || (app.data?.gstRegistered === 'Yes' ? 'Registered' : 'Not Registered')}</div>
                      </td>
                      <td className="p-4">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {app.status !== 'APPROVED' && (
                            <button
                              ...
                              className="px-3 py-1.5 bg-brand-forest text-white hover:bg-brand-forest/90 rounded-xl text-xs font-bold transition-all flex items-center gap-1 shadow-sm disabled:opacity-50"
                            >
