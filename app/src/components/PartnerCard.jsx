...
export default function PartnerCard({ partner }) {
  ...
  return (
    <div className="group relative bg-white rounded-2xl overflow-hidden border border-slate-200/80 hover:border-brand-forest/40 shadow-card-soft hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
      ...
      <div className="relative h-52 w-full overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-brand-forest/90 text-white backdrop-blur-md shadow-sm border border-white/15">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            <span>Verified Atelier</span>
          </span>

          {established && (
            <span className="px-2.5 py-1 rounded-full text-[11px] font-medium tracking-wider uppercase bg-slate-900/80 text-white backdrop-blur-md border border-white/10">
              {established}
            </span>
          )}
        </div>
        ...
        <div className="absolute bottom-3 left-3 text-xs">
          <span className="px-2.5 py-1 rounded-lg font-bold uppercase tracking-wider bg-brand-terracotta text-white shadow-sm">
            {cuisine}
          </span>
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col justify-between bg-white">
        <div>
          ...
          <div className="mb-2">
            <h3 className="font-serif text-xl sm:text-2xl font-bold text-slate-900 group-hover:text-brand-terracotta transition-colors leading-tight">
              {name}
            </h3>
            {partner.tagline && (
        ...
        <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
          <Link
            to={`/partners/${slug}`}
            className="btn-primary w-full py-2.5 text-xs uppercase tracking-wider flex items-center justify-center gap-1.5"
          >
